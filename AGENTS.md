...
- `extension/` - Chrome MV3 extension (context menu "Check/Report with Aghoy", paste-to-check popup). Minimal permissions (`contextMenus`, `activeTab`, `storage`, `notifications`; sole `host_permissions` is the Worker origin). Mirrors `storageClient` via `extension/api.js`; reports feed the same D1 database. Never add broad host permissions or content scripts that read pages.
...
- **R2 evidence storage is enabled.** The `EVIDENCE` binding is active in `wrangler.toml` (`[[r2_buckets]]`, bucket `project-aghoy-evidence`) and the evidence routes serve real R2 (verified live: write 200, read 200, auth 401, oversize 413, missing report 404). `scripts/setup-storage.sh enable-r2` is idempotent (no-op when already enabled). To re-provision or verify: run `bash scripts/setup-storage.sh enable-r2`, and keep a budget alert under Billing (R2 free tier: 10GB + 1M writes + 10M reads/mo).
